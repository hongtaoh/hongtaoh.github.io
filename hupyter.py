...
if __name__ == '__main__':
    ...
    notebook_folder_name = re.findall(
        r'^.+?(?=\/)', arg1)[0]  # e.g., 'notebooks'
    # the _files folder auto generated by nbconvert
    files_folder = re.sub('.ipynb', '_files', arg1)
    # https://stackoverflow.com/a/18710430
    files_folder_name = re.findall(
        f'^{notebook_folder_name}\/([^\s]+)', files_folder)[0]
    mdfile = re.sub('.ipynb', '.md', arg1)  # markdown file name

    move_to = 'content/' + arg2  # where post is located
    static_folder = 'static/' + arg2
    target_static_folder = static_folder + '/' + \
        files_folder_name  # the target static folder
    ...
    for lk in md_image_links:
        if 'https://' in lk:
            ...
            if files_folder_name not in lk:
                ...
                txt = re.sub(f'{image_folder_name}/',
                             f'/{arg2}/{files_folder_name}/', txt)
                os.system(
                    f'cp {notebook_folder_name}/{lk} {static_folder}/{files_folder_name}')
