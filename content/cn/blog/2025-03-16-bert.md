...
`$$\text{Cosine Similarity(A, B)} = \frac{\vec{A} \cdot \vec{B}}{|A||B|} = \vec{A} \cdot \vec{B} = \sum_{i=1}^{n}a_i b_i$$`
...
也就是说这两个是完全负相关的关系，余弦相似度越大，欧几里得距离越小。所以两者虽然具体数值不同，但是排序的结果一样。那为什么 OpenAI 推荐用余弦相似度呢？因为它的运算更少。我们来看：
...
'Stowaway',
'Space Chimps'],
array([0.56054974, 0.5572939 , 0.55047977, 0.5243796 , 0.4685108 ,
0.46683484, 0.4646523 , 0.463014  , 0.46231508, 0.45956627],
dtype=float32))
{{< /indentedblock >}}
结果正确。
...
我们可以用 [Pinecone](https://www.pinecone.io/)。首先，
...
{{< indentedblock >}}
Index 'sentence-transformer-movie-embedding' does not exist.
Uploaded batch 1/11
Uploaded batch 2/11
Uploaded batch 3/11
Uploaded batch 4/11
...
{{< indentedblock >}}
/var/folders/wx/xz5y_06d15q5pgl_mhv76c8r0000gn/T/ipykernel_3383/496837610.py:37: UserWarning: Failed to obtain server version. Unable to check client-server compatibility. Set check_version=False to skip version check.
qdrant_client = QdrantClient(
{{< /indentedblock >}}
在本地运行没问题：
...
embedding_model = genai.embed_content(model="models/gemini-embedding-001", content="Where is France?")

print(f"gemini-embedding-001 的 output 维度为 {len(embedding_model['embedding'])}")
{{< /codeCollapse >}}

{{< indentedblock >}}
gemini-embedding-001 的 output 维度为 3072
{{< /indentedblock >}}
我好奇 `gemini-embedding-001` 的结果是否已经归一化：
...
需要注意的是，谷歌的限制是每分钟 3000 个请求。不要超过。
...
max_workers = 2 # 同时进行 2 个线程
with ThreadPoolExecutor(max_workers=max_workers) as executor:
    results = list(tqdm(
        executor.map(lambda batch: process_one_batch(batch[0], batch[1], index), all_batches),
...
{{< indentedblock >}}
Uploading batches: 100%|██████████| 212/212 [13:29<00:00,  3.82s/it]

成功处理 212/212 批次
{{< /indentedblock >}}
13 分钟处理完。

![](/cn/blog/2025-03-16-bert_files/img/pinecone-movies-gemini.png)

接下来，我们来试一下搜索。需要重新写 `search_pinecone` 因为我们需要用 Google-Gemini 来给搜索文本一个向量表示，而不是之前的 `sentence_transformer` 模型。
...
[{'title': 'The Martian',
'overview': 'During a manned mission to Mars, Astronaut Mark Watney is presumed dead after a fierce storm and left behind by his crew. But Watney has survived and finds himself stranded and alone on the hostile planet. With only meager supplies, he must draw upon his ingenuity, wit and spirit to subsist and find a way to signal to Earth that he is alive.',
'similarity_score': 0.773887694},
{'title': 'Infinity 7',
'overview': 'A lone astronaut’s spacecraft malfunctions, threatening to leave him stranded in orbit forever.',
'similarity_score': 0.773116946},
{'title': 'Stowaway',
...
[{'title': 'The Martian',
'overview': 'During a manned mission to Mars, Astronaut Mark Watney is presumed dead after a fierce storm and left behind by his crew. But Watney has survived and finds himself stranded and alone on the hostile planet. With only meager supplies, he must draw upon his ingenuity, wit and spirit to subsist and find a way to signal to Earth that he is alive.',
'similarity_score': 0.77091223},
{'title': 'Infinity 7',
'overview': 'A lone astronaut’s spacecraft malfunctions, threatening to leave him stranded in orbit forever.',
'similarity_score': 0.758911},
{'title': 'Stowaway',
'overview': 'A three-person crew on a mission to Mars faces an impossible choice when an unplanned passenger jeopardizes the lives of everyone on board.',
'similarity_score': 0.746429443},
{'title': 'Passage to Mars',
'overview': 'A NASA Arctic expedition designed to be the first Martian road trip on Earth becomes an epic two-year odyssey of human adventure and survival.',
'similarity_score': 0.717178345},
{'title': 'The Space Between Us',
'overview': 'A young man raised by scientists on Mars returns to Earth to find his father.',
'similarity_score': 0.715705931}]
{{< /indentedblock >}}
结果也不错。说明 `gemini-embedding-001` 支持多语言。
...
当我们点开 AI Recommendations，点击搜索后，具体的流程就是，我们先在 `app/recommendation/page.js` 中点击，然后 `app/api/recommend/route.js` 就会运行，连带着运行 `lib/geminiEmbedding.js`、`lib/pinecone.js` 和 `lib/explain.js`。然后结果返回 `app/recommendation/page.js`。
