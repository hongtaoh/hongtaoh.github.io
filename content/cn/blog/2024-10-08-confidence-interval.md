...
我们把 `$t_{99}$` 的 `$97.5\%$` 百分位数 (percentile) 记为 `$c$`，也就是上图中红线所对应的 `$T = 1.98$`。其所代表的含义是所有的 `$T$` 中，落在红线以及红线往左这个区域的 `$T$` 占比为 `$97.5\%$`。也可以理解为，随机一个 `$T$` 落在红线以及红线往左这个区域的概率是 `$97.5\%$`
...
我们也可以这样记：`$\mu$` 的 `$1 - \alpha$` 置信区间为：

`$$P_{\mu} (\bar{X} - \frac{S}{\sqrt{n}} \cdot t_{n-1, \frac{\alpha}{2}} \le \mu \le \bar{X} + \frac{S}{\sqrt{n}} \cdot t_{n-1, 1- \frac{\alpha}{2}})$$`
...
从一个分布为 `$\mathcal N \sim(\mu, \sigma^2)$` 的母体中随机一个大小为 `$n$` 的样本，算出 `$\bar{X}$` 和 `$S$`，记 `$b$` 为 `$t_{n-1}$` 这一分布的 `$\frac{\alpha}{2}$` 百分位数 (percentile)，`$c$` 为 `$t_{n-1}$` 这一分布的 `$1 - \frac{\alpha}{2}$` 百分位数 (percentile)，并计算出这样一个区间

`$$\left[\bar{X} + \frac{b \cdot S}{\sqrt{n}}, \bar{X} + \frac{c \cdot S}{\sqrt{n}} \right]$$`

上面这个过程我们重复无数次，得到无数个这样的区间。那这些区间中，有 `$\left( 100 (1 - \alpha) \right) \%$` 包含了母体均值 `$\mu$`。

上面这个公式我换成了 `$b$` 和 `$c$`，而不是 `$-c$` 和 `$c$` 是因为不是所有的分布都对称，比如[卡方分布](/cn/2024/09/29/chi2/)。
