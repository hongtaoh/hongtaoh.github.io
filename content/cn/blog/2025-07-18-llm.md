...
它主要有两个模块：encoder（编码器）和 decoder（解码器）。Encoder 的作用是把输入文本变成一组上下文相关的向量表示 (contextualized embeddings)。这里一定要注意「上下文相关」这几个字。之前的 embedding，比如 GloVe 是静态的，但是最新的 embedding 模型比如 BERT 是上下文相关的。比如，经典的例子，bank。在不同的语境下，embedding 是不同的。
