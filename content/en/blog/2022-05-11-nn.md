...
![](/media/cnblog/nn/activation.png)
...
![](/media/cnblog/nn/hidden-layer.png)
...
![](/media/cnblog/nn/components.png)
...
![](/media/cnblog/nn/combination-subcomponents.png)
...
![](/media/cnblog/nn/small-edges.png)
...
![](/media/cnblog/nn/recognized-edges.png)
...
![](/media/cnblog/nn/surroundings-negative.png)
...
This way, what the activation of that neuron in the 2nd layer measures is how positive/negative the weighted sum is:

![](/media/cnblog/nn/apply-sigmoid-to-sum.png)
...
![](/media/cnblog/nn/many-biases.png)
...
![](/media/cnblog/nn/13k-weights-and-biases.png)
...
![](/media/cnblog/nn/recap-weights-bias.png)
...
![](/media/cnblog/nn/math-representation-of-calculating-one-2nd-layer-activation.png)
...
![](/media/cnblog/nn/math-representation-in-python.png)
...
![](/media/cnblog/nn/neural-network-as-a-function-01.png)

![](/media/cnblog/nn/neural-network-as-a-function-2.png)
...
![](/media/cnblog/nn/cost-illustration.png)
...
![](/media/cnblog/nn/cost-calculation.png)
...
![](/media/cnblog/nn/average-cost.png)
...
![](/media/cnblog/nn/cost-as-a-function.png)
...
![](/media/cnblog/nn/2nd-layer-bad-job.png)
...
![](/media/cnblog/nn/noise-image-confident-wrong-result.png)
...
Take the image of a handwritten `$2$` as an example. In the output layer, we want the activiation of that neuron (Let's call it `$n_{out2}$`) corresponding to `$2$` to be `$1.0$` and activiation of all other neurons to be `$0.0$`. Therefore, we want to increase the activation of `$n_{out2}$`, and decrease the activation of other neurons. The change of activations include both **direction** and **quantity**. For example, if the activation of the neuron corresponding to `$3$` is previously `$0.9$`, then we want it to decrease **a lot**. The activation of the neuron corresponding to `$8$` was `$0.2$`, so we only need it to increase a little bit. 

![](/media/cnblog/nn/desired-changes-in-output-layer.png)
...
![](/media/cnblog/nn/change-weights.png)
...
![](/media/cnblog/nn/desired-changes-in-activations.png)
...
![](/media/cnblog/nn/considerations-by-all-neurons.png)
...
![](/media/cnblog/nn/average-all-desired-changes.png)
...
![](/media/cnblog/nn/desired-changes-as-gradients.png)
